...
describe Bundler::Whatsup::ChangelogFetcher, :vcr do
  ...
  describe '#changelog_name' do
    ...
    context 'it should find changelog file when it name differs from CHANGELOG.md' do
      ...
      let(:trailblazer_gem_info) { double }

      before do
        allow(trailblazer_gem_info).to receive('[]').with('name').and_return('trailblazer')
        allow(trailblazer_gem_info).to receive('[]').with('source_code_uri').and_return('https://github.com/trailblazer/trailblazer')
        allow(trailblazer_gem_info).to receive('[]').with('homepage_uri').and_return(nil)
      end

      it 'finds CHANGES.md' do
        expect(described_class.new(trailblazer_gem_info).changelog_file_name).to eq('CHANGES.md')
      end
